import 'dart:convert';
import 'package:SeeWriteSay/dto/user_profile_response_dto.dart';
...
class UserApiService {
  static Future<bool> checkNicknameAvailable(String nickname) async {
    final token = await CommonLogicService.getToken(); // ✅ 토큰 불러오기
  ...
  static Future<String> generateRandomNickname() async {
    final token = await CommonLogicService.getToken(); // ✅ 토큰 필요 시 동일하게 적용
  ...
  static Future<UserProfileResponseDto> getCurrentUserProfile() async {
    ...
    if (response.statusCode == 200) {
      final json = jsonDecode(response.body);
      return UserProfileResponseDto.fromJson(json['data']);
    } else {
